...
<#global subject>[<@common.subjMarker/>, SUCCESSFUL] Build ${project.name}::${buildType.name} #${build.buildNumber}</#global>

<#global body>Build ${project.name}::${buildType.name} #${build.buildNumber} successful ${var.buildShortStatusDescription}.
Agent: ${agentName}
...
<div>
  <div>
    Build <i>${project.name?html}::${buildType.name?html} <a href='${link.buildResultsLink}'>#${build.buildNumber}</a></i> successful
